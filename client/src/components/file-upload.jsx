...
const FileUpload = (props) => {
   ...
   const onDrop = (files) => {
      ...
      axios.post("/api/auth/uploadimage", formData, config).then((res) => {
         setState({
            ...state,
            uploading: false,
            uploadedFiles: [...state.uploadedFiles, res.data],
         });
   ...
   const showUploadedImages = () =>
      state.uploadedFiles.map((i) => (
         <div
            key={i.public_id}
            onClick={() => onRemove(i.public_id)}
   ...
   return (
      <div>
         ...
         <div className='show_upload_image'>{showUploadedImages()}</div>
